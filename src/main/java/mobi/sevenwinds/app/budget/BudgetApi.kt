...
fun NormalOpenAPIRoute.budget() {
    route("/budget") {
        route("/add") {
            post<Unit, BudgetRecordResponse, BudgetRecordRequest>(info("Добавить запись")) { param, body ->
...
data class BudgetRecordResponse(
    ...
    @Min(1) val amount: Int,
    val type: BudgetType,
    var author: AuthorRecordResponse?,
) {
    constructor(
        year: Int,
        month: Int,
...
data class BudgetYearParam(
    ...
    @QueryParam("ФИО автора", allowEmptyValues = true) val fullName: String?
)
